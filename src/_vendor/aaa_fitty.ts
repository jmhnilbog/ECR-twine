...
var fitty: Fitty = (() => {
    ...
    const markAsClean = (f: { dirty: number }) => (f.dirty = DrawState.IDLE);

    const calculateStyles = (f: {
        availableWidth: number;
        element: { parentNode: { clientWidth: any }; scrollWidth: any };
        currentWidth: number;
    ...
    const subscribe = (f: { active: boolean }) => () => {
        if (f.active) return;
        f.active = true;
        requestRedraw();
    };
    ...
    const unsubscribe = (f: { active: boolean }) => () => (f.active = false);

    const observeMutations = (f: {
        observeMutations: any;
    ...
    function fittyCreate(elements: any[], options: {}) {
        // set options object
        ...
        // create fitties
        const publicFitties = elements.map((element: any) => {
    ...
    function fitty(target: string, options = {}) {
    ...
    let resizeDebounce: number | null | undefined = null;
    const onWindowResized = () => {
        clearTimeout(resizeDebounce as number);
